...
def load_module(graph_path, lib_path, params_path):
    temp = util.tempdir()
    loaded_json = open(temp.relpath(graph_path)).read()
    loaded_lib = tvm.module.load(lib_path)
    loaded_params = bytearray(open(temp.relpath(params_path), "rb").read())
...
if __name__ == '__main__':
    ...
    for hardware_path in hardware_path_list:
        ...
        temp = set()
        for file_path in file_path(hardware_path):
        ...
        for path in temp:
            ...
            print('model: {}, time: {} us({} us)'.format(key, mean_time, std_time))
    
    print("model \t time(us)")

    for key, time in time_list:
        model_name = key.split('_')[-1]
        print("{} \t {}({})".format(model_name, time[0], time[1]))
